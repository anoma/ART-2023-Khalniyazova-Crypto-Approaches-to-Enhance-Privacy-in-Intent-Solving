...
TEE has some advantages (e.g., performance, latency) compared to other cryptographic solutions, like multiparty computations and homomorphic encryption, which are often considered in similar contexts, and is used by multiple projects to provide privacy \citep{flash} \citep{secret}.
...
A downside of using TEE is that it is infamous for being broken regularly, multiple hardware attacks have been discovered over the years \citep{sgxfail} \citep{downfall}, along with other threats like side-channel attacks and covert channels \citep{flash}, when untrusted inputs might alter the program execution and leak private information. For these reasons, many consider TEE only helpful for defence in-depth but not as the main security component to rely on.
...
In \citep{fe}, a functionality $F$ is defined as a function $F: K \times X \rightarrow \{0, 1\}$ described as a \textit{deterministic Turing machine} and the decryption function is defined such that $dec(sk, c) = y$, where $sk$ is the secret key, $c$ is the encrypted message, and $y$ is the output of the function.
